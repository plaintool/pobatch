...
uses
  ...
  SysUtils,
  Contnrs,
  pocheck;   // QA check options, messages and the TPOChecker class

  // All QA-related resourcestrings, the TQACheckOptions record and the
  // TPOChecker class now live in the pocheck unit.
...
type
  ...
  public
    ...
    // Return the effective QA options with PluralFormsCount filled in from
    // the Plural-Forms header. The stored FQACheckOptions are not modified.
...
begin
  ...
  begin
    ...
    FormMsgs := TPOChecker.RunOnText(FMsgId, FormText, AOptions);
